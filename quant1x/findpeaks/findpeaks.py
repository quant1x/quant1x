...
import quant1x.findpeaks.interpolate as interpolate
import quant1x.findpeaks.stats as stats
from quant1x.findpeaks.stats import disable_tqdm
...
class findpeaks():
    ...
    def __init__(self, method=None, whitelist=['peak', 'valley'], lookahead=200, interpolate=None, limit=None,
                 imsize=None, scale=True, togray=True, denoise='fastnl', window=3, cu=0.25,
                 params_caerus={'window': 50, 'minperc': 3, 'nlargest': 10, 'threshold': 0.25}, figsize=(15, 8),
                 verbose=3):
    ...
    # %% Plotting
    def plot(self, legend=True, figsize=None, cmap=None, text=True, xlabel='x-axis', ylabel='y-axis', prefix=''):
        """Plot results.
        ...
        if self.args['type'] == 'peaks1d':
            fig_axis = self.plot1d(legend=legend, figsize=figsize, xlabel=xlabel, ylabel=ylabel, prefix=prefix)
    ...
    def plot1d(self, legend=True, figsize=None, xlabel='x-axis', ylabel='y-axis', prefix=''):
        """Plot the 1D results.
    ...
    def plot_mesh(self, wireframe=True, surface=True, rstride=2, cstride=2, cmap=plt.cm.hot_r, title='', figsize=None,
                  view=None, savepath=None):
        """Plot the 3d-mesh.
        ...
        if surface:
            ...
            ax2 = fig.gca()
            ax2.plot_surface(xx, yy, self.results['Xproc'], rstride=rstride, cstride=cstride, cmap=cmap, linewidth=0,
    ...
    def plot_persistence(self, figsize=(20, 8), fontsize_ax1=14, fontsize_ax2=14, xlabel='x-axis', ylabel='y-axis',
        ...
        ax1, ax2 = self._plot_persistence_ax1(fontsize_ax1, ax1, ax2, figsize, xlabel, ylabel, verbose, prefix=prefix)
        # Plot
        plt.show()
        # Return
        return ax1, ax2

    def _plot_persistence_ax1(self, fontsize, ax1, ax2, figsize, xlabel, ylabel, verbose, prefix=''):
        title = 'Persistence'
        if len(prefix) > 0:
            title = prefix + ' / ' + title
        if self.args['type'] == 'peaks1d':
            ...
            ax1 = _plot_original(self.results['df']['y'].values, self.results['df']['x'].values,
                                 ...
                                 title=title, figsize=figsize, legend=True, ax=ax1, xlabel=xlabel,
                                 ylabel=ylabel)
            # Set limits
