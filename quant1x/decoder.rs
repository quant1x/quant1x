...
impl CalendarDecoder {
    pub fn new(_data: &str) -> Self {
        ...
        for i in 0..26 { base64_chars.push((65 + i) as u8 as char); }
        for i in 0..26 { base64_chars.push((97 + i) as u8 as char); }
        for i in 0..10 { base64_chars.push((48 + i) as u8 as char); }
        base64_chars.push('+'); base64_chars.push('/');

        let mut h = vec![0i64; 64];
        for i in 0..64 { h[i] = 1i64 << i; }

        CalendarDecoder {
            indices: Vec::new(), e: 0, o: 0, n: 0,
            base64_chars,
            r: HashMap::new(),
            h,
            s: 0,
    ...
    fn y(&mut self) -> bool {
        if self.e >= self.n { return false; }
        let t = (self.indices[self.e] & (1 << self.o)) != 0;
        self.o += 1;
        if self.o >= 6 { self.o -= 6; self.e += 1; }
        t
    }
    ...
    fn w(&mut self, t: &[i32], r_param: Option<&[i32]>, a_param: Option<&[i32]>) -> Vec<i64> {
        ...
        for (i, &ti) in t.iter().enumerate() {
            ...
            let mut u: i64 = 0;
            if c != 0 {
                if self.e >= self.n { for j in 0..l.len() { l[j] = 0; } return l; }
                if c <= 30 {
                    while c > 0 {
                        if self.e >= self.n { for j in 0..l.len() { l[j] = 0; } return l; }
    ...
    pub fn decode_json(&mut self) -> Value {
        ...
        match self.branch_type {
            1479 | 136 => {
                ...
                for r in rows {
                    let mut map = Map::new();
                    if let Some(d) = r.date { map.insert("date".to_string(), Value::String(d)); }
                    if let Some(v) = r.open { map.insert("open".to_string(), Value::String(v)); }
                    if let Some(v) = r.high { map.insert("high".to_string(), Value::String(v)); }
                    if let Some(v) = r.low { map.insert("low".to_string(), Value::String(v)); }
                    if let Some(v) = r.close { map.insert("close".to_string(), Value::String(v)); }
                    if let Some(v) = r.volume { map.insert("volume".to_string(), Value::String(v)); }
            ...
            200 => {
                ...
                for r in rows {
                    ...
                    if let Some(d) = r.date { map.insert("date".to_string(), Value::String(d)); }
                    if let Some(v) = r.close { map.insert("close".to_string(), Value::String(v)); }
                    if let Some(v) = r.prevclose { map.insert("prevclose".to_string(), Value::String(v)); }
                    arr.push(Value::Object(map));
                }
                Value::Array(arr)
            }
            ...
            197 => {
                ...
                let mut arr = Vec::new();
                for row in data {
                    let inner = row.into_iter().map(|v| Value::Number(serde_json::Number::from(v))).collect();
    ...
    pub fn t_branch(&mut self) -> Vec<Record> {
        ...
        loop {
            if self.e >= self.n { break; }
            if loop_guard > 1_000_000 { break; }
            loop_guard += 1;
            ...
            if (32 & (a6[0] as i64)) != 0 {
                loop {
                    ...
                    if 63 == (16 | a2v) {
                        let l: String;
                        if (16 & a2v) != 0 { l = "x".to_string(); } else { l = "u".to_string(); }
                        let a3 = self.w(&[3,3], None, None);
                        i_map.insert(format!("{}_d", l), a3.get(0).copied().unwrap_or(0) + *self.r.get("md").unwrap_or(&0));
                        i_map.insert(format!("{}_v", l), a3.get(1).copied().unwrap_or(0) + *self.r.get("mv").unwrap_or(&0));
                        break;
                    }
                    if (32 & a2v) != 0 {
                        let o: String;
                        if (8 & a2v) != 0 { o = "d".to_string(); } else { o = "v".to_string(); }
                        let l: String;
                        if (16 & a2v) != 0 { l = "x".to_string(); } else { l = "u".to_string(); }
                        i_map.insert(format!("{}_{}", l, o), (7 & a2v) as i64 + *self.r.get(&format!("m{}", o)).unwrap_or(&0));
                        break;
                    }
                    let o = (15 & a2v) as i64;
                    if o == 0 {
                        ...
                        if tmp.len() > 0 { self.r.insert("d".to_string(), tmp[0]); }
            ...
            let mut low = open;
            let mut closev = open;
            if dvals.len() > 1 { high = (cd as f64 + dvals[1] as f64) / m; }
            if dvals.len() > 2 { low = (cd as f64 - dvals[2] as f64) / m; }
            if dvals.len() > 3 { closev = (cd as f64 + dvals[3] as f64) / m; self.r.insert("cd".to_string(), cd + dvals[3]); }
    ...
    pub fn s_branch(&mut self) -> Vec<Record> {
        ...
        if self.s >= 1 { return result; }
        let init = self.w(&[18], Some(&[1]), None);
        if init.len() == 0 { return result; }
        ...
        self.r.insert("pc".to_string(), *self.r.get("cd").unwrap_or(&0));
        ...
        loop {
            ...
            result.push(rec);
            if self.e >= self.n || (self.e == self.n-1 && (63 & (*self.r.get("c").unwrap_or(&0) ^ tcounter)) == 0) {
                break;
            }
            tcounter += 1;
        }
    ...
    pub fn mi_run(&mut self) -> Vec<Vec<i64>> {
        ...
        if wf.len() == 0 { return result; }
        let rf = wf[0] as usize;
        let wc = self.w(&[6], None, None);
        if wc.len() == 0 { return result; }
        let rc = wc[0] as i64;
    ...
    pub fn k_list(&mut self) -> Vec<String> {
        ...
        if dstart.len() > 0 { self.r.insert("d".to_string(), dstart[0]-1); }
        let target = self.w(&[18], None, None);
        let target_date = if target.len()>0 { target[0] as i64 } else { 0 };
        while *self.r.get("d").unwrap_or(&0) < target_date {
            let current_date = self.x(1);
            if n_count <= 0 {
                if self.y() { let v = self.n_bits(); let l = self.r.entry("l".to_string()).or_insert(0); *l += v as i64; }
                let count_data = self.w(&[3 * (*self.r.get("l").unwrap_or(&0) as i32)], Some(&[0]), None);
    ...
    pub fn intraday(&mut self) -> Vec<Record> {
        ...
        if self.s > 2 { return c; }
        // init
        let init = self.w(&[18], Some(&[1]), None);
        if init.len() == 0 { return c; }
        self.r.insert("d".to_string(), init[0]-1);
        let header = if self.s < 1 { self.w(&[3,3,4,1,1,1,5], None, None) } else { self.w(&[4,4,4,1,1,1,3], None, None) };
        ...
        self.r.insert("m".to_string(), m);
        let mut t = 0i64;
        while !(self.e >= self.n) && !(self.e == self.n-1 && (7 & (*self.r.get("c").unwrap_or(&0) ^ t)) == 0) {
            ...
            for i in 0..3 {
                let pkey = ["v","p","a"][i];
                let mut has = true;
                if pkey == "v" && *self.r.get("tv").unwrap_or(&0) == 0 { has = false; }
                if has && self.y() { let v = self.n_bits(); let key = format!("l{}", pkey); let cur = self.r.entry(key).or_insert(0); *cur += v as i64; }
                let mut u = 1;
                if pkey=="v" && *self.r.get("rv").unwrap_or(&0) != 0 {
                    if self.y() { u = 1; } else { u = 0; }
                }
                let mut extra = 0;
                if pkey=="v" && u==1 { extra = 7; }
                let call_len = 3 * (*self.r.get(&format!("l{}", pkey)).unwrap_or(&0) as i32) + extra;
                let vals = self.w(&[call_len], Some(&[if i!=0 {1} else {0}]), None);
                let mut val = if vals.len()>0 { vals[0] } else { 0 };
                if u==0 { val *= 100; }
                o_map.insert(pkey, val);
                if pkey=="v" { total_v += val; if val==0 && (*self.r.get("zv").unwrap_or(&0) !=0 || !self.y()) { o_map.insert("p", 0); } }
            }
        ...
        if c.len()>0 {
            ...
            c[0].prevclose = Some(format!("{:.6}", (*self.r.get("pc").unwrap_or(&0) as f64) / (*self.r.get("m").unwrap_or(&1) as f64)));
        }
        c
    }
}
