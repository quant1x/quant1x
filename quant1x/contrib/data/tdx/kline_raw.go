...
func (d *DataKLineRaw) Update(code exchange.InstrumentInfo, _date exchange.Timestamp) {
	...
	log.Debugf("[dataset::KLineRaw] [%s]: from %s to %s", code, currentStartDate.OnlyDate(), currentEndDate.OnlyDate())
	...
	if err := data.SaveKlineRaw(cacheFilename, klines); err != nil {
		log.Errorf("[dataset::KLineRaw] save error: %v", err)
	}
}
...
func init() {
	...
	if err := data.Register(&DataKLineRaw{}); err != nil {
		log.Errorf("[dataset::KLineRaw] failed to register plugin: %v", err)
	}
}
