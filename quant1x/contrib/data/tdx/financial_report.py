...
from quant1x.config import reports_filename
...
def _parse_quarterly_report_item(v: dict) -> QuarterlyReport:
    """Parse a single quarterly report item from API response."""
...
def quarterly_reports(feature_date: str, page_no: int = 1) -> Tuple[List[QuarterlyReport], int, Optional[FinancialReportException]]:
    ...
    try:
        ...
        reports = [_parse_quarterly_report_item(v) for v in data_list]
...
def quarterly_reports_by_security_code(security_code: str, date: str, diff_quarters: int, page_no: int = 1) -> Tuple[List[QuarterlyReport], int, Optional[FinancialReportException]]:
    """Fetch quarterly reports for a specific security code."""
...
def cache_quarterly_reports_by_security_code(date: str, diff_quarters: int = 1) -> Tuple[List[QuarterlyReport], int, Optional[FinancialReportException]]:
    ...
    if os.path.exists(filename):
        try:
            ...
            logger.debug(f"Read CSV error: {e}")

    # Fetch from network
    qdate = date
    if diff_quarters > 1:
        _, _, tmp_date = get_quarter_by_date(date, diff_quarters - 1)
        qdate = tmp_date
    ...
    try:
        ...
        logger.debug(f"Save CSV error: {e}")
...
def get_cache_quarterly_reports_by_security_code(security_code: str, date: str, diff_quarters: int = 1) -> Optional[QuarterlyReport]:
    ...
    # 1. 从内存缓存查找
    if normalized_code in _g_map_quarterly_reports:
        return _g_map_quarterly_reports[normalized_code]
    ...
    for diff in range(diff_quarters, 5):
        ...
        # 内存缓存
        if filename in _map_reports:
            for v in _map_reports[filename]:
                if v.SecurityCode == normalized_code:
                    _g_map_quarterly_reports[normalized_code] = v
                    return v
        ...
        if os.path.exists(filename):
            try:
                df = pd.read_csv(filename)
                for _, row in df.iterrows():
                    if row.get("SecurityCode") == normalized_code:
                        r = QuarterlyReport()
                        for field_name in r.__dataclass_fields__:
                            if field_name in row:
                                val = row[field_name]
                                if pd.isna(val):
                                    if isinstance(getattr(r, field_name), (int, float)):
                                        ...
                                        val = ""
                                setattr(r, field_name, val)
                        _g_map_quarterly_reports[normalized_code] = r
                        return r
            except Exception:
                pass
...
def _is_index(security_code: str) -> bool:
    """Check if security code is an index (not a stock)."""
    try:
        inst = detect_symbol(security_code)
        from quant1x.data.meta.instrument import InstrumentType
        return inst.type == InstrumentType.INDEX
    except Exception:
        return False


def get_quarterly_report_summary(security_code: str, date: str) -> QuarterlyReportSummary:
    """Get quarterly report summary for a security code."""
    summary = QuarterlyReportSummary()

    if _is_index(security_code):
        return summary
