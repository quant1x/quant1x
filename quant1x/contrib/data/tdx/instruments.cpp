...
//
// tdx/instruments - TDX security cache loader and initializer
// Aligned with Python contrib/data/tdx/instruments.py and Rust instruments.rs
...
bool load_securities() {
    ...
    try {
        ...
        while (in.read_row(exchange_str, type_str, ticker, name, lot_str, prec_str, extm_str, extc_str, alias)) {
            try {
                quant1x::data::meta::Instrument inst;
                inst.exchange = quant1x::data::meta::exchange_from_abbr(exchange_str);
                inst.type = quant1x::data::meta::instype_from_string(type_str);
...
static std::vector<quant1x::data::meta::Instrument> fetch_security_list(quant1x::data::meta::Exchange exchange, int start, int count) {
    ...
    int market_id = helpers::exchange_to_market(exchange);
    if (market_id < 0) {
        spdlog::error("[tdx/instruments] unsupported exchange for std market: {} (code={})",
                      quant1x::data::meta::exchange_label(exchange), quant1x::data::meta::exchange_code(exchange));
        return result;
    }
...
static void write_securities_csv(const std::string& fname, const std::vector<quant1x::data::meta::Instrument>& instruments) {
    ...
    try {
        ...
        for (const auto& inst : instruments) {
            writer.write_row(
                quant1x::data::meta::exchange_identifier(inst.exchange),
                quant1x::data::meta::instype_to_string(inst.type),
                inst.ticker,
                inst.name,
                inst.lot_size,
...
// ============================================================
// do_init_securities() - actual initialization logic invoked by RollingOnce::Do
// ============================================================
static void do_init_securities() {
    ...
    std::vector<quant1x::data::meta::Exchange> std_markets = {
        ...
        quant1x::data::meta::Exchange::SZSE,
        quant1x::data::meta::Exchange::BSE,
    };

    for (auto m : std_markets) {
        int start = 0;
        std::vector<quant1x::data::meta::Instrument> rows;
        while (true) {
        ...
        spdlog::info("[tdx/instruments] fetched {} instruments from {}", rows.size(),
                     quant1x::data::meta::exchange_label(m));
