...
namespace encoding {
    ...
    inline void deserialize_to_object(const json_t &node, T &obj, bool strict = false);
        template<typename T>
        inline void deserialize_vector(const json_t &node, T &vec);
        template<typename T>
        inline void deserialize_map(const json_t &node, T &map);
    ...
    inline void deserialize_field(const json_t &node, T &field, bool strict = false) {
        ...
        inline void serialize_field(json_t &node, const T &field) {
            if constexpr (is_optional_v<T>) {
                if (field.has_value()) serialize_field(node, field.value());
