...
impl BinaryStream {
    ...
    fn check_available(&self, required: usize) -> Result<(), crate::base::DeserializeError> {
        ...
        {
            Err(crate::base::DeserializeError::Truncated {
                needed: required,
                available,
            })
        } else {
    ...
    // primitive reads
    pub fn get_u8(&mut self) -> Result<u8, crate::base::DeserializeError> {
        self.check_available(1)?;
        let v = self.buffer[self.offset];
        self.offset += 1;
        Ok(v)
    }
    pub fn get_i8(&mut self) -> Result<i8, crate::base::DeserializeError> {
        Ok(self.get_u8()? as i8)
    }

    pub fn get_u16(&mut self) -> Result<u16, crate::base::DeserializeError> {
    ...
    pub fn get_i16(&mut self) -> Result<i16, crate::base::DeserializeError> {
        Ok(self.get_u16()? as i16)
    }

    pub fn get_u32(&mut self) -> Result<u32, crate::base::DeserializeError> {
    ...
    pub fn get_i32(&mut self) -> Result<i32, crate::base::DeserializeError> {
        Ok(self.get_u32()? as i32)
    }

    pub fn get_u64(&mut self) -> Result<u64, crate::base::DeserializeError> {
    ...
    pub fn get_i64(&mut self) -> Result<i64, crate::base::DeserializeError> {
        Ok(self.get_u64()? as i64)
    }

    pub fn get_f32(&mut self) -> Result<f32, crate::base::DeserializeError> {
        Ok(f32::from_bits(self.get_u32()?))
    }
    pub fn get_f64(&mut self) -> Result<f64, crate::base::DeserializeError> {
    ...
    pub fn get_byte_array(&mut self, out: &mut [u8]) -> Result<(), crate::base::DeserializeError> {
        let n = out.len();
    ...
    pub fn get_length_prefixed_string(&mut self) -> Result<String, crate::base::DeserializeError> {
        let len = self.get_u32()? as usize;
        self.check_available(len)?;
        let s = String::from_utf8(self.buffer[self.offset..self.offset + len].to_vec())
            .map_err(|_| crate::base::DeserializeError::InvalidUtf8)?;
        self.offset += len;
        Ok(s)
    }

    // raw string with fixed len (truncate at first NUL)
    pub fn get_string(&mut self, len: usize) -> Result<String, crate::base::DeserializeError> {
        self.check_available(len)?;
    ...
    pub fn varint_decode(&mut self) -> Result<i64, crate::base::DeserializeError> {
        let mut pos = self.offset;
        if pos >= self.buffer.len() {
            return Err(crate::base::DeserializeError::Truncated {
                needed: 1,
                available: 0,
            });
        }
