...
func tdxFetchRawSecurityBars(securityCode string, category level1.KLineType, start, count uint16) ([]level1.SecurityBar, error) {
...
func tdxUpdateKLine(code string, _date exchange.Timestamp) {
	...
	// 1. 确定缓存文件并读取本地缓存
	cacheFilename := config.GetKlineFilename(sc.String(), true)
	var cacheKLines []data.KLine
	err := encoding.CsvToSlices(cacheFilename, &cacheKLines)
	if err != nil {
		log.Printf("[DataKLine] load cache failed for %s: %v", code, err)
		// 继续更新
	}
	...
	if klinesLength > 0 {
		...
		log.Printf("[DataKLine] [%s]: cached klines=%d, adjustTimes=%d, start from %s", code, klinesLength, adjustTimes, currentStartDate.OnlyDate())
	}
