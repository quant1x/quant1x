	...
	match crate::level1::client::client() {
		Ok(mut pooled) => {
			...
			match crate::level1::process_request(pooled.stream(), req_buf.as_slice()) {
				...
				Err(e) => {
					log::error!("level1 process_request error for block_info {} offset {}: {}", filename, offset, e);
					None
				}
			}
		}
		Err(e) => {
			log::error!("failed to acquire level1 client for block_info {} offset {}: {}", filename, offset, e);
			None
		}
	}
}
