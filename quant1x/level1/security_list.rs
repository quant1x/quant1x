			...
			Ok(mut pooled) => {
				...
				match crate::level1::process_request(pooled.stream(), req_buf.as_slice()) {
					Ok(body) => {
						...
						log::info!("level1::security_list - market={} start={} count={}", market, start, resp.count);
						Some(resp)
					}
					Err(e) => {
						log::error!("level1 process_request error for security_list market={} start={}: {}", market, start, e);
						None
					}
				}
			}
