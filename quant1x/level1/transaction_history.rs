...
pub fn fetch_history_transactions(security_code: &str, date: u32, start: u16, count: u16) -> Option<TransactionHistoryResponse> {
	match crate::level1::client::client() {
		Ok(mut pooled) => {
			...
			match crate::level1::process_request(pooled.stream(), req_buf.as_slice()) {
				Ok(body) => {
					let mut resp = TransactionHistoryResponse::new(req.market as i32, &String::from_utf8_lossy(&req.code));
					resp.deserialize(&body);
					Some(resp)
				}
				Err(e) => {
					log::error!("level1 process_request error for history_transaction {} date {}: {}", security_code, date, e);
					None
				}
			}
		}
		Err(e) => {
			log::error!("failed to acquire level1 client for history_transaction {} date {}: {}", security_code, date, e);
			None
		}
	}
}
